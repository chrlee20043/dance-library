...
export default function AllVideos({ token, userId }) {
  ...
  const [selectedStyle, setSelectedStyle] = useState(""); // Track the selected style
  ...
  // Get a list of unique styles from the videos
  ...
  const videosToDisplay = videos.filter((video) => {
    ...
    // Check if the video matches the selected types, search parameter, and selected style
    return (
      typeMatch &&
